...
# Loading weights
model.load_state_dict(torch.load(f'./saved_model/VT_AE_MNist_withgaussian' + '.pt'))
G_estimate.load_state_dict(torch.load(f'./saved_model/G_estimate_MNist_withgaussian' + '.pt'))
...
def Patch_Overlap_Score(threshold, data_load=loader[1:], upsample=1):
    ...
    for n, data in enumerate(data_load):
        ...
        for c, (i, j) in enumerate(data):
            ...
            if n == 1 and counter_anom < 10:
                # Plot of reconstructions
                tensor1 = reconstructions.squeeze()  # Entfernen Sie die erste Dimension mit Länge 1
                tensor1 = np.transpose(tensor1.cpu().detach().numpy(),
                                       (1, 2, 0))  # Ändern Sie die Reihenfolge der Achsen
                # Plot of Image
                tensor2 = i.squeeze()
                tensor2 = np.transpose(tensor2.cpu().detach().numpy(),
                                       (1, 2, 0))  # Ändern Sie die Reihenfolge der Achsen
                # Plot erstellen
                counter_anom += 1
            ...
            if c < 10:
                ...
                fig.savefig(f'plot_{typ}_{counter}_Anomaly_detection.png')
...
if __name__ == "__main__":
    ...
    print(f'Loss3_ta mean: {np.mean(loss3_ta)}, Loss3_ta_std: {np.std(loss3_ta)}')
    print(f'Loss3_tn mean: {np.mean(loss3_tn)}, Loss3_tn_std: {np.std(loss3_tn)}')
